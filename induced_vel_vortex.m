...
for i=1:N
        ...
        if norm(cross(AB,AP))==0 && abs(x)>=abs(x_b) && abs(x)<=abs(x_c) && abs(y)>abs(y_b) && abs(y)<abs(y_c)
            V_ab = [0;0;0];
        else
        ...
        if vort_p(5,i,1) == 0          
            ...
            if norm(cross(CD,CP))==0 && abs(x)>=abs(x_b) && abs(x)<=abs(x_c) && abs(y)>abs(y_b) && abs(y)<abs(y_c)
                V_cd = [0;0;0];
            else
